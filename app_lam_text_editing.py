...
def demo_lam(flametracking, lam, cfg):
    ...
    def core_fn(image_path: str, video_params, working_dir, enable_oac_file, text_edit_prompt, text_edit_strength):
        image_raw = os.path.join(working_dir.name, "raw.png")
        ...
        # MVP: text command -> conservative FLAME parameter deltas.
        # Examples: "笑一点", "张嘴", "眨眼", "看左", "脸瘦一点".
        # This edits the driving FLAME sequence only; it does NOT rerun avatar reconstruction.
        if text_edit_prompt and str(text_edit_prompt).strip():
            motion_seq["flame_params"] = apply_text_edit(
                motion_seq["flame_params"],
                str(text_edit_prompt),
                strength=float(text_edit_strength),
                verbose=True,
            )
    ...
    with gr.Blocks(analytics_enabled=False) as demo:
        ...
        with gr.Row():
            with gr.Column(variant='panel', scale=1):
                ...
                text_edit_prompt = gr.Textbox(
                    label="Text Edit Prompt / 文本编辑指令",
                    placeholder="例如：笑一点、张嘴、眨眼、看左、脸瘦一点",
                    value="",
                )
                text_edit_strength = gr.Slider(
                    label="Edit Strength / 编辑强度",
                    minimum=0.0,
                    maximum=1.0,
                    value=0.5,
                    step=0.05,
        ...
        submit.click(
            ...
            inputs=[input_image, video_input,
                    working_dir, enable_oac_file, text_edit_prompt, text_edit_strength],  # video_params refer to smpl dir
